...
shopt -s nullglob
for llvm_config in llvm-config-$version llvm-config-${version}.0 llvm-config${version}0 llvm-config-mp-$version llvm-config-mp-${version}.0 llvm${version}-config $brew_llvm_config llvm-config-${version}-32 llvm-config-${version}-64 llvm-config; do
